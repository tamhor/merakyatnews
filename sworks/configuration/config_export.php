...
<?php if($forward == 'barang'){ ?>
  ...
  $query1="select barang.kode as kode, barang.nama as nama, terjual, terbeli, sisa, kategori.nama as kategori from $forward left join kategori on kategori.kode = barang.kategori where barang.kode like '%$search%' or barang.nama like '%$search%' or kategori.nama like '%$search%' order by barang.no";
				$hasil = mysqli_query($conn,$query1);
				$no = 1;
				while ($fill = mysqli_fetch_assoc($hasil)){
					?>
...
<?php if($forward == 'beli'){ ?>
  ...
  $query1="SELECT *, supplier.nama as supplier FROM  beli left join supplier on supplier.kode = beli.supplier where nota like '%$search%' or tglbeli like '%$search%' or supplier.nama like '%$search%' order by beli.no desc";
				$hasil = mysqli_query($conn,$query1);
				$no = 1;
				while ($fill = mysqli_fetch_assoc($hasil)){
					?>
